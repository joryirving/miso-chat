...
function initSchema() {
  ...
  // Sessions ownership tracking table
  db.exec(`
...
const sessions = {
  // Record session ownership (first user to access becomes the owner)
  setOwner(sessionKey, userId) {
    db.prepare(
      'INSERT OR IGNORE INTO sessions (session_key, user_id) VALUES (?, ?)'
    ).run(sessionKey, userId);
  },

  // Check if a user has access to a session
  hasAccess(sessionKey, userId) {
    const row = db.prepare(
      'SELECT user_id FROM sessions WHERE session_key = ? AND user_id = ?'
  ...
  getOwner(sessionKey) {
    const row = db.prepare('SELECT user_id FROM sessions WHERE session_key = ?').get(sessionKey);
    return row ? row.user_id : null;
  },
};

module.exports = { db, reactions, sessions };
