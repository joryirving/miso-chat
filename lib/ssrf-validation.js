...
/**
 * Return the first private/loopback/link-local address in a list, or null.
 */
function findPrivateAddress(addresses) {
  for (const ip of addresses || []) {
    if (typeof ip !== 'string') continue;
    if (isPrivateIPv4(ip) || isPrivateIPv6(ip)) return ip;
  }
  return null;
}

/**
 * Resolve a hostname once and validate the answer, returning the address the
 * caller must dial. Closes the DNS time-of-check/time-of-use race (issue #849):
 * the caller connects to the returned address instead of letting the HTTP
 * client perform a fresh, unvalidated resolution.
 *
 * @param {string} hostname - Hostname (or IP literal) to resolve and validate.
 * @param {object} [options]
 * @param {AbortSignal} [options.signal] - Optional abort signal for the lookup.
 * @param {Function} [options.lookup] - Resolver override (tests); must honor `{ all: true }`.
 * @returns {Promise<{address: string, addresses: string[]}>} The validated address to pin.
 * @throws {Error} Error with `code === 'EPRIVATEHOST'` if any answer is private.
 */
async function resolveValidatedLinkPreviewAddress(hostname, options = {}) {
  const privateHostError = () => {
  ...
  const normalized = String(hostname || '').toLowerCase().replace(/^\[/, '').replace(/\]$/, '');
  if (!normalized) throw privateHostError();

  if (net.isIP(normalized)) {
    if (isPrivateIPv4(normalized) || isPrivateIPv6(normalized)) throw privateHostError();
    return { address: normalized, addresses: [normalized] };
  }

  const lookupFn = options.lookup || dns.promises.lookup;
  const lookupOptions = { verbatim: true, all: true };
  if (options.signal) lookupOptions.signal = options.signal;
  const result = await lookupFn(normalized, lookupOptions);
  ...
  if (!addresses.length) {
    throw new Error(`DNS resolution returned no usable addresses for ${normalized}`);
  }

  const privateAddress = findPrivateAddress(addresses);
  if (privateAddress) {
    const err = privateHostError();
    err.address = privateAddress;
    throw err;
  }
