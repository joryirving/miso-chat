 ...
 */
// Reactions are emoji; anything longer is malformed input, not an emoji.
...
function createReactionsRoutes({ isAuthenticated, requireSessionAccess, authMode, reactions }) {
  ...
  router.post('/messages/:messageId/reactions', isAuthenticated, requireSessionAccess(authMode), (req, res) => {
    try {
      ...
      if (typeof rawEmoji !== 'string' || rawEmoji.length > MAX_EMOJI_LENGTH) {
        return res.status(400).json({ error: 'Emoji must be a string of at most 64 characters' });
      }
