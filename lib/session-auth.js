...
function requireSessionAccess(authMode) {
  return function sessionAccessMiddleware(req, res, next) {
    ...
    // In 'none' mode, any authenticated user can access any session
    if (authMode === 'none') {
      return next();
    }

    // In 'local'/'oidc' mode, check session ownership
    const sessionKey = req.params.key || req.body?.session_key;
    if (!sessionKey) {
      return res.status(400).json({ error: 'Session key is required' });
    }

    // Record ownership on first access (sessions are created externally by OpenClaw)
    sessions.setOwner(sessionKey, req.user.id);

    if (!sessions.hasAccess(sessionKey, req.user.id)) {
